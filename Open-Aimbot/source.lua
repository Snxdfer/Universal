...
function InterfaceManager:ImportSettings()
    pcall(function()
        if not DEBUG and getfenv().isfile and getfenv().readfile and getfenv().isfile("UISettings.OAB") and getfenv().readfile("UISettings.OAB") then
            for Key, Value in next, HttpService:JSONDecode(getfenv().readfile("UISettings.OAB")) do
...
function InterfaceManager:ExportSettings()
    pcall(function()
        if not DEBUG and getfenv().isfile and getfenv().readfile and getfenv().writefile then
            getfenv().writefile("UISettings.OAB", HttpService:JSONEncode(UISettings))
        end
    end)
end
...
pcall(function()
    if not DEBUG and getfenv().isfile and getfenv().readfile and getfenv().isfile(string.format("%s.OAB", game.GameId)) and getfenv().readfile(string.format("%s.OAB", game.GameId)) and UISettings.AutoImport then
        ImportedConfiguration = HttpService:JSONDecode(getfenv().readfile(string.format("%s.OAB", game.GameId)))
...
do
    if typeof(script) == "Instance" and script:FindFirstChild("Fluent") and script:FindFirstChild("Fluent"):IsA("ModuleScript") then
        ...
        local Success, Result = pcall(function()
            return game:HttpGet("https://raw.githubusercontent.com/Snxdfer/Universal/refs/heads/main/Open-Aimbot/Fluent.lua", true)
        end)
        if Success and typeof(Result) == "string" and string.find(Result, "dawid") then
            ...
            if Fluent.Premium then
                return getfenv().loadstring(game:HttpGet("https://raw.githubusercontent.com/Snxdfer/Universal/refs/heads/main/Open-Aimbot/Aimbot.lua", true))()
            end
            local Success, Result = pcall(function()
                return game:HttpGet("https://raw.githubusercontent.com/Snxdfer/Universal/refs/heads/main/Open-Aimbot/AimbotStatus.json", true)
            end)
...
do
    ...
    if getfenv().isfile and getfenv().readfile and getfenv().writefile and getfenv().delfile then
        ...
        ConfigurationManager:AddButton({
            ...
            Callback = function()
                xpcall(function()
                    if getfenv().isfile(string.format("%s.OAB", game.GameId)) and getfenv().readfile(string.format("%s.OAB", game.GameId)) then
                        local ImportedConfiguration = HttpService:JSONDecode(getfenv().readfile(string.format("%s.OAB", game.GameId)))
                        for Key, Value in next, ImportedConfiguration do
                        ...
                        Window:Dialog({
                            ...
                            Content = string.format("Configuration File %s.OAB has been successfully loaded!", game.GameId),
                            Buttons = {
                                {
                                    Title = "Confirm"
                                }
                        ...
                        Window:Dialog({
                            Title = "Configuration Manager",
                            Content = string.format("Configuration File %s.OAB could not be found!", game.GameId),
                            Buttons = {
                                {
                                    Title = "Confirm"
                                }
                    ...
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("An Error occurred when loading the Configuration File %s.OAB", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
        ...
        ConfigurationManager:AddButton({
            ...
            Callback = function()
                xpcall(function()
                    ...
                    getfenv().writefile(string.format("%s.OAB", game.GameId), ExportedConfiguration)
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("Configuration File %s.OAB has been successfully overwritten!", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
                    ...
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("An Error occurred when overwriting the Configuration File %s.OAB", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
        ...
        ConfigurationManager:AddButton({
            ...
            Callback = function()
                if getfenv().isfile(string.format("%s.OAB", game.GameId)) then
                    getfenv().delfile(string.format("%s.OAB", game.GameId))
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("Configuration File %s.OAB has been successfully removed!", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
    ...
    if UISettings.ShowWarnings then
        ...
        elseif ShowWarning then
            ...
            Window:Dialog({
                Title = string.format("%s 💫PREMIUM💫", string.format(MonthlyLabels[os.date("*t").month], "Open Aimbot")),
                Content = "✨Upgrade to unlock all Options✨ – Contact ??? via Discord to buy",
                Buttons = {
                    {
                        Title = "Confirm"
                    }
...
local OnTeleport; OnTeleport = Player.OnTeleport:Connect(function()
    if DEBUG or not Fluent or not getfenv().queue_on_teleport then
        ...
        getfenv().queue_on_teleport("getfenv().loadstring(game:HttpGet(\"https://raw.githubusercontent.com/Snxdfer/Universal/refs/heads/main/Open-Aimbot/source.lua\", true))()")
        OnTeleport:Disconnect()
    end
end)
